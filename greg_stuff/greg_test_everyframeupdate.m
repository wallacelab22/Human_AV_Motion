...
% Stimulus variables
coherence = 0.05; %values from 0-1
coh_string = num2str(coherence);
aperture_diameter = 30; %degrees visual angle, given monWidth = 40cm and Viewing Distance from monitor = 53cm
velocity = 20; %degrees per second
vel_string = num2str(velocity);
motion_direction = 0; %0=right, 180=left, 90=up, 270=down
dir_string = num2str(motion_direction);
...
% RDK Dot variables
writer = VideoWriter(sprintf('RDK_%sdir_%scoh_%svel.mp4', dir_string, coh_string, vel_string), 'MPEG-4');
