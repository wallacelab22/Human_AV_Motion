function [rightward_prob] = multisensory_rightward_prob_calc(right_vs_left, right_group, left_group, right_var, left_var)
...
for i = max(left_group):-1:1
    ...
    logical_array = group_rows(:, 5) == left_var;
...
if size(right_vs_left, 1) >= 3 && size(right_vs_left{3,1}, 2) >= 2
    ...
    logical_array = group_rows(:, 5) == right_var;
    count = sum(logical_array);
