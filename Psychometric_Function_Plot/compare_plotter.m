...
if compare_stairVis == 1
    ...
    rightward_prob = unisensory_rightward_prob_calc(right_vs_left, right_group, left_group, right_var, left_var);
    [total_coh_frequency, left_coh_vals, right_coh_vals, coherence_lvls, coherence_counts, coherence_frequency] = frequency_plotter(data_output, right_vs_left)
    [fig, p_values, ci, threshold, xData, yData, x, p, sz] = normCDF_plotter(coherence_lvls, rightward_prob, ...
    chosen_threshold, left_coh_vals, right_coh_vals, coherence_frequency, ...
    compare_plot, save_name);
...
if compare_psyAud == 1
    ...
    [right_vs_left, right_group, left_group] = direction_plotter(data_output);
    rightward_prob = unisensory_rightward_prob_calc(right_vs_left, right_group, left_group, right_var, left_var);
    [left_coh_vals, right_coh_vals, coherence_lvls, coherence_frequency] = frequency_plotter(data_output, right_vs_left);
    [xData, yData, x, p, sz] = normCDF_plotter(coherence_lvls, rightward_prob, ...
    chosen_threshold, left_coh_vals, right_coh_vals, coherence_frequency, ...
    compare_plot, save_name);
    scatter(xData, yData, sz, 'LineWidth', 2, 'MarkerEdgeColor', 'r');
    hold on
