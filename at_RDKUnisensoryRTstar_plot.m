...
for kk = 1:5 % 5 Acoh levels not including 0, i.e. no catch trials, looping over each Acoh levels
    ...
    Audy(kk) = nanmedian(AudAcoh(:,6))/((length(find(AudAcoh(:,1)==AudAcoh(:,5)))/length(AudAcoh))); % %correct responses for each Acoh levels not including catch trials
end
...
for kk = 1:5 % 5 Acoh levels not including 0, i.e. no catch trials, looping over each Acoh levels
    ...
    Visy(kk) = nanmedian(VisAcoh(:,6))/((length(find(VisAcoh(:,1)==VisAcoh(:,5)))/length(VisAcoh))); % %correct responses for each Acoh levels not including catch trials
end
...
for vv = 1:2 % 2 Vcoh levels not including 0, i.e. no catch trials, looping over each Vcoh levels
    ...
    Audv(vv) = nanmedian(AudVcoh(:,6))/(length(find(AudVcoh(:,3)==AudVcoh(:,5)))/length(AudVcoh)); % %correct responses for each Acoh levels not including catch trials
end
...
for vv = 1:2 % 2 Vcoh levels not including 0, i.e. no catch trials, looping over each Vcoh levels
    ...
    Visv(vv) = nanmedian(VisVcoh(:,6))/(length(find(VisVcoh(:,3)==VisVcoh(:,5)))/length(VisVcoh)); % %correct responses for each Acoh levels not including catch trials
end
...
for kk=1:5
    ...
    Aud_ylow(kk) = nanmedian(Aud_Acoh_Vlow(:,6))/(length(find(Aud_Acoh_Vlow(:,1)==Aud_Acoh_Vlow(:,5)))/length(Aud_Acoh_Vlow)); % correct trials
    Aud_yhigh(kk) = nanmedian(Aud_Acoh_Vhigh(:,6))/(length(find(Aud_Acoh_Vhigh(:,1)==Aud_Acoh_Vhigh(:,5)))/length(Aud_Acoh_Vhigh));
end
...
for kk=1:5
    ...
    Aud_ylowi(kk) = nanmedian(Aud_Acoh_Vlowi(:,6))/(length(find(Aud_Acoh_Vlowi(:,1)==Aud_Acoh_Vlowi(:,5)))/length(Aud_Acoh_Vlowi)); % correct trials
    Aud_yhighi(kk) = nanmedian(Aud_Acoh_Vhighi(:,6))/(length(find(Aud_Acoh_Vhighi(:,1)==Aud_Acoh_Vhighi(:,5)))/length(Aud_Acoh_Vhighi));
end
...
for kk=1:5
    ...
    Vis_ylow(kk) = nanmedian(Vis_Acoh_Vlow(:,1))/(length(find(Vis_Acoh_Vlow(:,3)==Vis_Acoh_Vlow(:,5)))/length(Vis_Acoh_Vlow)); % correct trials
    Vis_yhigh(kk) = nanmedian(Vis_Acoh_Vhigh(:,6))/(length(find(Vis_Acoh_Vhigh(:,3)==Vis_Acoh_Vhigh(:,5)))/length(Vis_Acoh_Vhigh));
end
...
for kk=1:5
    ...
    Vis_ylowi(kk) = nanmedian(Vis_Acoh_Vlowi(:,6))/(length(find(Vis_Acoh_Vlowi(:,3)==Vis_Acoh_Vlowi(:,5)))/length(Vis_Acoh_Vlowi)); % correct trials
    Vis_yhighi(kk) = nanmedian(Vis_Acoh_Vhighi(:,6))/(length(find(Vis_Acoh_Vhighi(:,3)==Vis_Acoh_Vhighi(:,5)))/length(Vis_Acoh_Vhighi));
end
