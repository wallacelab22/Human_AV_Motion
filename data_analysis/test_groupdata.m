...
for participant = 1:num_participants
    ...
    if isstruct(current_data) && isfield(current_data, 'data_output')
        ...
        for level = 1:length(coherence_levels)
            ...
            if condition == 3
                correct_responses = sum(trials(:, 8));
            else
            ...
            if condition == 3
                hits = sum(trials(:, 8));
                false_alarms = size(trials, 1) - sum(trials(:, 8));
            else
            ...
            % Store in matrix
            d_prime_matrix(participant, level, condition) = d_prime;
...
% You can now analyze the multisensory_gain structure for your research
 ...
 % Calculate multisensory gain for each coherence level
    for level = 1:length(coherence_levels)
        ...
        accuracy_matrix(participant, level, 1) = auditory_accuracy(level);
        accuracy_matrix(participant, level, 2) = visual_accuracy(level);
        accuracy_matrix(participant, level, 3) = av_accuracy(level);
        multisensory_gain(participant, level) = gain;
    end
end


AUD_dprime = d_prime_matrix(:,:,1);
AUD_dprime = AUD_dprime(:, 3:end);
VIS_dprime = d_prime_matrix(:,:,2);
VIS_dprime = VIS_dprime(:, 3:end);
AV_dprime = d_prime_matrix(:,:,3);
AV_dprime = AV_dprime(:, 3:end);

% Create a heatmap
figure;  % Create a new figure
imagesc(AUD_dprime);  % Create the heatmap
colormap('jet');  % Choose a colormap, e.g., 'hot', 'jet', etc.
colorbar;  % Add a colorbar to show the mapping of data values to colors

% Add labels and titles
xlabel('Coherence');
ylabel('Participant');
title('Auditory D Prime Scores Heatmap');
set(gca, 'XTick', 1:7, 'XTickLabel', {'0.0625', '0.0884', '0.1250', '0.1768', '0.2500', '0.3536', '0.5000'});
set(gca, 'YTick', 1:9, 'YTickLabel', arrayfun(@num2str, 1:10, 'UniformOutput', false));
% Set the color scale range for the heatmap
clim([-0.5, 8.5]);
...
[numParticipants, numConditions] = size(AUD_dprime);
for i = 1:numParticipants
    for j = 1:numConditions
        text(j, i, num2str(AUD_dprime(i, j), '%0.2f'), ...
...
% Create a heatmap
figure;  % Create a new figure
imagesc(VIS_dprime);  % Create the heatmap
colormap('jet');  % Choose a colormap, e.g., 'hot', 'jet', etc.
colorbar;  % Add a colorbar to show the mapping of data values to colors
...
xlabel('Coherence');
ylabel('Participant');
title('Visual D Prime Scores Heatmap');
set(gca, 'XTick', 1:7, 'XTickLabel', {'0.0625', '0.0884', '0.1250', '0.1768', '0.2500', '0.3536', '0.5000'});
set(gca, 'YTick', 1:9, 'YTickLabel', arrayfun(@num2str, 1:10, 'UniformOutput', false));
% Set the color scale range for the heatmap
clim([-0.5, 8.5]);
...
% Optionally, adjust the aspect ratio
pbaspect([1 1.5 1]);  % Adjust aspect ratio7 as needed

% Display the values on the heatmap
[numParticipants, numConditions] = size(VIS_dprime);
for i = 1:numParticipants
    for j = 1:numConditions
        text(j, i, num2str(VIS_dprime(i, j), '%0.2f'), ...
             'HorizontalAlignment', 'center', ...
             'Color', 'white');
    end
end
...
% Create a heatmap
figure;  % Create a new figure
imagesc(AV_dprime);  % Create the heatmap
colormap('jet');  % Choose a colormap, e.g., 'hot', 'jet', etc.
colorbar;  % Add a colorbar to show the mapping of data values to colors
% Set the color scale range for the heatmap
clim([-0.5, 8.5]);
...
xlabel('Coherence');
ylabel('Participant');
title('Audiovisual D Prime Scores Heatmap');
set(gca, 'XTick', 1:7, 'XTickLabel', {'0.0625', '0.0884', '0.1250', '0.1768', '0.2500', '0.3536', '0.5000'});
set(gca, 'YTick', 1:9, 'YTickLabel', arrayfun(@num2str, 1:10, 'UniformOutput', false));
...
% Display the values on the heatmap
[numParticipants, numConditions] = size(AV_dprime);
for i = 1:numParticipants
    for j = 1:numConditions
        text(j, i, num2str(AV_dprime(i, j), '%0.2f'), ...
             'HorizontalAlignment', 'center', ...
             'Color', 'white');
    end
end
...
colors = cool(num_participants); % 'lines' colormap provides a good range of distinct colors

% Loop to plot each participant's data
for participant = 1:num_participants
    plot(selected_coherence_levels, multisensory_gain(participant, 3:9), '-o', ...
         'Color', colors(participant, :), 'LineWidth', 3);  % Plot with lines connecting dots
...
title('MS Gain across Coherences for All Participants');
set(findall(gcf, '-property', 'FontSize'), 'FontSize', 44)
legend(arrayfun(@(x) sprintf('Participant %d', x), 1:num_participants, 'UniformOutput', false), 'FontSize', 14);
